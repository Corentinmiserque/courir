<template>
    <div class="homepage-info">
      <div class="program" v-for="(program, programIndex) in data.programs" :key="program.programID">
        ...
        <!-- Utiliser find pour obtenir la première semaine non terminée -->
        <div v-if="program.weeks.find(week => !week.finished)">
          <!-- Stocker la semaine active dans une variable -->
          <div v-if="activeWeek = program.weeks.find(week => !week.finished)">
            <div class="week">
              ...
              <div id="day-progress">
                <!-- Utiliser countFinishedDaysInWeek avec activeWeek -->
              ...
              <div v-for="(day, dayIndex) in activeWeek.days" :key="day.dayID" :class="{ finished: day.finished }">
                <RouterLink :to="{ name: 'parcours', params: { id: day.dayID } }">Jour {{ dayIndex + 1 }}</RouterLink>
              </div>
            </div>
          </div>
        </div>
  ...
  const data = ref(programStore.data);
  
  const saveDataToLocalStorage = () => {
    localStorage.setItem('courirData', JSON.stringify(data.value));
  };
